...
- [x] Android (API 23+, NDK AMidi + JNI)
...
### Android
- Requires Android API 23+ and the Android NDK (r20b+). Use cargo-ndk for cross-compiling and packaging your .so.
- No extra Java/Kotlin is required: device enumeration and opening is handled via JNI with a dynamic proxy.
- Build (example):
...
- Permissions/features:
  - Manifest should declare `<uses-feature android:name="android.software.midi" android:required="false" />` (not for USB/BLE MIDI).
